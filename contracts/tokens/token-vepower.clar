...
(define-fungible-token vepower)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "vePower")
(define-data-var token-symbol (string-ascii 10) "vepower")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/vepower.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? vepower amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance vepower who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply vepower))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? vepower amount recipient)
	)
)
