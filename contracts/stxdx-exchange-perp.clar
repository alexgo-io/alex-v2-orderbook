				...
				(
					...
					(settle-per-fill 
						(if left-buy ;; left => liquidation (of buy), linked => initiating (of sell)
							(if (<= (+ margin-per-fill linked-order-take) left-order-make) u0 (- (+ margin-per-fill linked-order-take) left-order-make))
							(if (<= (+ margin-per-fill right-order-make) linked-order-make) u0 (- (+ margin-per-fill right-order-make) linked-order-make))
						)
					)
				)
				(map-set 
		...
		(if (is-eq (len (get linked-hash right-order)) u0)
			...
			(let 
				...
				(
					...
					(margin-per-fill (get margin-per-fill linked-order))					
					(settle-per-fill 
						(if right-buy ;; right => liquidation (of buy), linked => initiating (of sell)
							(if (<= (+ margin-per-fill linked-order-take) right-order-make) u0 (- (+ margin-per-fill linked-order-take) right-order-make))
							(if (<= (+ margin-per-fill left-order-make) linked-order-make) u0 (- (+ margin-per-fill left-order-make) linked-order-make))                            
						)
					)
				)
				(map-set 
